...
import Database.Arschloch.ArschlochViewModel;
import Database.Arschloch.Arschloch_Partie;
import Database.Arschloch.Arschloch_Spieler;
import recyclerview.arschloch_adapter;

public class Arschloch extends Fragment {
    private ArschlochViewModel viewModel;
    public Integer j = 0;
    List<Arschloch_Spieler> arschloch_spieler;
    public Arschloch_Partie arschloch_partien = null;
    static RecyclerView mRecyclerview;
    public arschloch_adapter adapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {   //Fragment Preambel
        ...
        getActivity().setTitle("Arschloch");
        ...
        adapter = new arschloch_adapter(new View.OnClickListener() {

            @Override
            public void onClick(View view) {
                for (int k = 0; k < viewModel.getAllArschloch_Partien().getValue().size(); k++) {
                    if (arschloch_adapter.id.equals(viewModel.getAllArschloch_Partien().getValue().get(k).getPartiebezeichnung())) {
                        arschloch_spieler = new ArrayList<>();
                        int s = 0;
                        for (int i = 0; i < viewModel.getAllArschloch_Partien().getValue().get(k).getSpieler().size(); i++) {
                            if (!viewModel.getAllArschloch_Partien().getValue().get(k).getSpieler().get(i).getName().equals("")) {
                                s++;
                            }
                        }
                        ...
                        for (int i = 0; i < viewModel.getAllArschloch_Partien().getValue().get(k).getSpieler().size(); i++) {
                            if (!viewModel.getAllArschloch_Partien().getValue().get(k).getSpieler().get(i).getName().equals("")) {
                                namen[s] = viewModel.getAllArschloch_Partien().getValue().get(k).getSpieler().get(i).getName();
                                s++;
                            }
                        }
                        input_punkte(viewModel.getAllArschloch_Partien().getValue().get(k), 0, namen.length, namen);
                    }
                }
            }
            ...
            public void onClick(View view) {
                for (int k = 0; k < viewModel.getAllArschloch_Partien().getValue().size(); k++) {
                    if (arschloch_adapter.id.equals(viewModel.getAllArschloch_Partien().getValue().get(k).getPartiebezeichnung())) {
                        ...
                        update_partie(viewModel.getAllArschloch_Partien().getValue().get(k));
                    }
                }
            }
        });
        ...
        viewModel = ViewModelProviders.of(getActivity()).get(ArschlochViewModel.class);
        viewModel.getAllArschloch_Partien().observe(getActivity(), new Observer<List<Arschloch_Partie>>() {
            @Override
            public void onChanged(@Nullable List<Arschloch_Partie> arschloch_parties) {adapter.submitList(arschloch_parties);
            }
        });
        ...
        new ItemTouchHelper(new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
            ...
            public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
                ...
                viewModel.delete(viewModel.getAllArschloch_Partien().getValue().get(viewHolder.getAdapterPosition()));
            }
        }}).attachToRecyclerView(mRecyclerview);
        return rootView;
    }
    //ACHTUNG_ Pfusch am Werk :D
    void new_players(int nummer, List<String> arschloch_spieler) {
        if (nummer < 8) {
            ...
            dialogBuilder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int whichButton) {
                    arschloch_spieler.add(input.getText().toString());
                    j++;
                    new_players(j, arschloch_spieler);
                }
            });
        ...
        } else new_List(arschloch_spieler);
    }
    //ACHTUNG_ Pfusch am Werk :D
    void new_List(final List<String> arschloch_spieler) {
        ...
        dialogBuilder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int whichButton) {
                if (!input.getText().toString().equals("")) {
                    List<Arschloch_Spieler> spieler = new ArrayList<>();
                    List<Integer> platzierungen = new ArrayList<>();
                    for (int k = 0; k < arschloch_spieler.size(); k++) {
                        if(!arschloch_spieler.get(k).equals("")){
                          platzierungen.add(0);
                        }
                    }
                    for (int k = 0; k <  arschloch_spieler.size(); k++) {
                        spieler.add(new Arschloch_Spieler(arschloch_spieler.get(k),platzierungen));
                    }
                    viewModel.add(new Arschloch_Partie(input.getText().toString(), spieler));
                    j = 0;
                } else {
                    Toast toast = Toast.makeText(getActivity(), "Bitte einen gültigen Partienamen eingeben", Toast.LENGTH_LONG);
                    toast.show();
                    new_List(arschloch_spieler);
                }
            }
        });
        dialogBuilder.setNegativeButton("Abbrechen", new DialogInterface.OnClickListener() {
    ...
    public void input_punkte(Arschloch_Partie arschloch_partie,  int iteration, int size, String[] namen) {
        if(iteration<size){
        ...
        builder.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialogInterface, int i) {
                List<Arschloch_Spieler> spieler =new ArrayList<>();
                for (int j = 0; j < arschloch_partie.getSpieler().size(); j++){
                    List<Integer> platzierungen = new ArrayList<>();
                    if(arschloch_partie.getSpieler().get(j).getPlatzierungen()!=null){
                    for (int k = 0; k < arschloch_partie.getSpieler().get(j).getPlatzierungen().size(); k++) {
                        if (iteration ==  k && namen[nb.getValue()].equals(arschloch_partie.getSpieler().get(j).getName())){
                            platzierungen.add(arschloch_partie.getSpieler().get(j).getPlatzierungen().get(k) + 1);
                        } else
                            platzierungen.add(arschloch_partie.getSpieler().get(j).getPlatzierungen().get(k));
                    }
                    spieler.add(new Arschloch_Spieler(arschloch_partie.getSpieler().get(j).getName(), platzierungen));
                }else spieler.add(new Arschloch_Spieler("",null));}
                if(iteration==size){
                }
               ...
               input_punkte(new Arschloch_Partie(arschloch_partie.getPartiebezeichnung(),spieler),iteration+1,size,update_namen);
            }
        });
        ...
        if(neg!=null) neg.setTextColor(getResources().getColor(R.color.textcolorDark));}
    else viewModel.update(arschloch_partie);}
    ...
    public void delete_alert(Activity activity, int position) {
            ...
            builder.setPositiveButton("Ja", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialogInterface, int i) {
                    viewModel.delete(viewModel.getAllArschloch_Partien().getValue().get(position));
    ...
    public void update_partie(Arschloch_Partie partie){
        CharSequence[] optionen = {"Namen der Partie", "Namen der Spieler", "Platzierungen der Spieler"};
    ...
    public void update_titel(Arschloch_Partie partie){
        MaterialAlertDialogBuilder builder = new MaterialAlertDialogBuilder(getActivity());
        ...
        builder.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
                viewModel.add(new Arschloch_Partie(edit.getText().toString(),partie.getSpieler()));
    ...
    public void update_name(Arschloch_Partie partie){
        int s=0;
                ...
                alert.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        for (int j = 0; j < partie.getSpieler().size() ; j++) {
                            if(j==selected[0]){
                                spieler.add(new Arschloch_Spieler(edit.getText().toString(), partie.getSpieler().get(j).getPlatzierungen()));
                            } else  spieler.add(new Arschloch_Spieler(partie.getSpieler().get(j).getName(), partie.getSpieler().get(j).getPlatzierungen()));
                        }
                        viewModel.update(new Arschloch_Partie(partie.getPartiebezeichnung(),spieler));
                    }
                });
    ...
    public void update_punkte(Arschloch_Partie partie){
        int s=0;
        final int[] selected = { 0 };
        for (int i = 0; i < partie.getSpieler().size() ; i++) {
            if(!partie.getSpieler().get(i).getName().equals("")){s++;}
        ...
        builder.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialogInterface, int i) {
                ...
                CharSequence[] n = new CharSequence[partie.getSpieler().get(selected[0]).getPlatzierungen().size()];
                if(partie.getSpieler().get(selected[0]).getPlatzierungen().size()==8){
                    n[0] = "King";
                    n[1] = "Vize-King";
                    n[2] = "1. Bauer";
                    n[3] = "2. Bauer";
                ...
                if(partie.getSpieler().get(selected[0]).getPlatzierungen().size()==7){
                    n[0] = "King";
                    n[1] = "Vize-King";
                    n[2] = "1. Bauer";
                    n[3] = "2. Bauer";
                ...
                if(partie.getSpieler().get(selected[0]).getPlatzierungen().size()==6){
                    n[0] = "King";
                    n[1] = "Vize-King";
                    n[2] = "1. Bauer";
                    n[3] = "2. Bauer";
                ...
                if(partie.getSpieler().get(selected[0]).getPlatzierungen().size()==5){
                    n[0] = "King";
                    n[1] = "Vize-King";
                    n[2] = "Bauer";
                    n[3] = "Vize-Arsch";
                ...
                if(partie.getSpieler().get(selected[0]).getPlatzierungen().size()==4){
                    n[0] = "King";
                    n[1] = "Vize-King";
                    n[2] = "Vize-Arsch";
                    n[3] = "Arschloch";
                }
                if(partie.getSpieler().get(selected[0]).getPlatzierungen().size()==3){
                    n[0] = "King";
                    n[1] = "1. Bauer";
                    n[2] = "Arschloch";
                }
                if(partie.getSpieler().get(selected[0]).getPlatzierungen().size()==2){
                    n[0] = "King";
                    n[1] = "Arschloch";
                }
                if(partie.getSpieler().get(selected[0]).getPlatzierungen().size()==1){
                    n[0] = "King";
                }
                for (int k = 0; k < partie.getSpieler().get(selected[0]).getPlatzierungen().size() ; k++) {

                    punkte[k]= n[k] +": " + partie.getSpieler().get(selected[0]).getPlatzierungen().get(k);
                }
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        alertDialogBuilder.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialogInterface, int i) {
                                ...
                                for (int j = 0; j <partie.getSpieler().size() ; j++) {
                                    if(j==selected[0]){
                                        spieler.add(new Arschloch_Spieler(partie.getSpieler().get(j).getName(),punkte));
